...
interface SheetProps {
  ...
  isEditing: boolean;
  expenseItem: ExpenseData | undefined;
  incomesItem: IncomesData | undefined;
}
...
export default function EditSheet({
  ...
  const { editIncomes } = useIncomesStore();
      ...
      <Transition appear show={open} as={Fragment}>
        <Dialog
          ...
          onClose={() => {
            setOpen(false);
            setIsExpense(false);
            setAmount(isExpense ? expenseItem?.amount : incomesItem?.amount);
          }}
        >
          <Transition.Child
            as={Fragment}
                ...
                >
                  <Dialog.Panel className="pointer-events-auto w-[500px] h-full bg-white shadow-xl p-6 border border-gray-200 dark:bg-[#0f172a] text-black dark:text-white transition-colors duration-500">
                    <Dialog.Title className="text-[28px] font-bold mt-5 flex justify-between">
                      {isExpense && isEditing ? "Edit Expense" : "Edit Incomes"}
