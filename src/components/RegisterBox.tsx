      ...
      <div className="flex flex-col gap-2">
        ...
        />
          {error === "auth/weak-password"
              &&
              <div className="flex items-center max-w-80 max-h-6 -mt-2 rounded-sm ml-3 border border-red-500  text-red-500">
                  <p className="flex ml-2 text-md">Password should be at least 6 characters!</p>
              </div>
          }
        ...
        <input
          ...
          className="flex outline-none border border-gray-500 rounded-xl min-w-104 min-h-13 p-2 m-2"
        />
          {error2 === "Password doesn’t match with the Confirm Password"
              &&
              <div className="flex items-center max-w-80 min-h-8 -mt-2 rounded-sm ml-3 border border-red-500 text-red-500">
                  <p className="flex ml-2 text-md">Confirm Password doesn’t match with the  Password!</p>
              </div>
          }
      ...
      <button
        onClick={() => {
            if(confirmPass === password){
                ...
                setError2("Password doesn’t match with the Confirm Password")
            }
            if(userCredentials){
